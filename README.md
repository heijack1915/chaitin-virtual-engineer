...
1. 打开 [Releases 页面](https://git.in.chaitin.net/zhenchao.zhang/chaitin-virtual-engineer/-/releases)，下载两个文件：
   - `chaitin-ve.exe`（主程序）
...
打开 [Releases 页面](https://git.in.chaitin.net/zhenchao.zhang/chaitin-virtual-engineer/-/releases)，下载对应系统的文件：
...
| **雷池管理** | 点「雷池管理」→ 填 API 地址和 Token → 直接在网页上管理 WAF 站点、策略、IP 组、日志 |
...
设置页面里有主流模型的**一键预设按钮**，点了就自动填好地址和模型名，你只需填 API Key。

**推荐模型（指令遵循能力强）：** OpenAI GPT-4o、Anthropic Claude、智谱 GLM-4、阿里通义千问
...
需要官方产品知识库（如雷池SafeLine完整用户手册、部署方案、监控配置等），请联系作者获取。

---

## 雷池 WAF 管理

通过 SafeLine OpenAPI 直接在网页上管理雷池 WAF，无需登录雷池管理界面。

### 使用方法

1. 在雷池管理界面获取 API Token（管理后台 → 系统设置 → OpenAPI）
2. 打开「雷池管理」标签页
3. 填写 API 地址（如 `https://192.168.1.10:9443`）和 API Token
4. 点「测试连接」确认通过后点「保存配置」

### 管理功能

| 功能 | 说明 |
|------|------|
| **防护总览** | 总请求数、攻击次数、拦截次数、拦截率，攻击类型 TOP 10 分布 |
| **站点管理** | 查看站点列表，新建站点（含域名、后端、防护策略、健康检查），启停/删除站点 |
| **IP 组** | 黑名单/白名单 IP 组管理，支持 CIDR 批量添加 |
| **攻击日志** | 攻击检测日志查询，支持搜索过滤和分页浏览 |
| **系统信息** | 产品版本、许可证信息、节点状态 |

> **提示：** API 地址和 Token 保存后下次打开自动加载，不用重复填写。配置文件存储在数据目录的 `safeline_config.json` 中。
...
| 雷池 WAF 管理 | 通过 OpenAPI 管理雷池站点、策略、IP 组、攻击日志，查看防护总览和系统信息 |
...
│   └── safeline/           # 雷池 WAF API 客户端
│       ├── client.go       # SafeLine OpenAPI 封装
│       └── helpers.go      # 辅助函数
├── models/
│   └── models.go           # 数据模型
├── ui/                     # 前端页面
...
### v1.1.0
- 新增「雷池管理」标签页，通过 SafeLine OpenAPI 直接管理 WAF
  - 防护总览：总请求/攻击/拦截统计，攻击类型 TOP 10
  - 站点管理：新建站点（域名、后端、防护策略、健康检查）、启停、删除
  - IP 组管理：黑名单/白名单，CIDR 批量添加
  - 攻击日志：分页查询，搜索过滤
  - 系统信息：产品版本、许可证、节点状态
- 连接配置自动持久化，下次打开无需重复填写
- 支持 SafeLine 自签名证书（自动跳过 TLS 验证）

### v1.0.1
- UI 文件打包进程序，不再需要外部 ui/ 文件夹
- AI 对话改进：强化指令遵循、防止编造执行结果、兼容更多命令格式
- 设置页面新增更多模型预设（12 个主流平台一键配置）
- 设置页面标注推荐模型和风险提示
- 知识库内容大幅扩充（用户操作手册从 332 行增加到 63889 字符）
